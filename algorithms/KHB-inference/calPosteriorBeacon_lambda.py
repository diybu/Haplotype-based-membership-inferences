...
def postEquation(logpin, logpabs, logpPrior, logpPriorabs):
	...
	pseudo = 10
        ...
        try:
                flambda = math.exp(logpin + logpPrior - (logpabs + logpPriorabs))
        except:
                if logpin + logpPrior - (logpabs + logpPriorabs) < 1:
                        flambda = 1e-06
                else:
                        flambda = 1e06
	return flambda
	...
	for idx in lwo:
		...
		print 'ltypeSymbol[idx]', ltypeSymbol[idx], idx
                if ltypeSymbol[idx]==0.0:
            		...
            		flambda = postEquation(logpin, logpabs, logpPriorin, logpPriorabs)
            		#print 'fppost', fppost
            		lpos[idx] = flambda
...
if __name__ == '__main__':
	...
	path = '/data2/diybu/beaconHaplopAttack/haploblock/beaconDB/solutionOuts_backup/'	
        ...
        for root, dirs, files in os.walk(path):
			...
			output = open('/data2/diybu/beaconHaplopAttack/haploblock/beaconDB/posteriorResult_lambda/beaconDB_'+sout, 'wb')
